                ...
                <table class="table table-striped mt-4">
                    ...
                    <tbody>
                        ...
                        try {
                            ...
                            // Query untuk mengambil data mata pelajaran berdasarkan hari
                            $stmt = $pdo->prepare("SELECT id_pelajaran, nama_pelajaran, nama_guru, jam, tingkat, deskripsi, hari FROM matapelajaran ORDER BY hari");
                            $stmt->execute();

                            // Inisialisasi variabel untuk menyimpan hari saat iterasi pertama
                            $currentDay = null;

                            // Loop melalui hasil query dan tampilkan dalam tabel
                            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                                // Cek apakah hari berubah
                                if ($row['hari'] != $currentDay) {
                                    // Jika hari berubah, tampilkan baris baru untuk nama hari
                                    echo "<tr><td colspan='5' class='text-center'><strong>" . htmlspecialchars($row['hari']) . "</strong></td></tr>";
                                    $currentDay = $row['hari']; // Simpan hari saat ini
                                }
                                // Tampilkan data mata pelajaran
                                echo "<tr>";
                                echo "<td></td>"; // Kolom untuk hari (dibiarkan kosong karena sudah ditampilkan di atas)
                    ...
                    <table>
                        ...
                        if ($result->rowCount() > 0) {
                            while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                                ...
                                echo "<td data-label='Aksi'><a href='mapel/index.php?id=" . $row["id_pelajaran"] . "'>Edit</a> | <a href='mapel/delete.php?id=" . $row["id_pelajaran"] . "'>Hapus</a></td>";
                                echo "</tr>";
                            }
                        } else {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <form method="POST" action="murid/update_user.php" style="display:inline;" onsubmit="validateForm(event);">
                                            ...
                                            <button type="submit" class="btn btn-success" style="background-color: #88c1d0;">Update</button>
                                        </form>
                                        <form method="POST" action="murid/delete_user.php" style="display:inline;">
                                            ...
                                            <button type="submit" class="btn btn-danger" style="background-color: #88c1d0;">Hapus</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
